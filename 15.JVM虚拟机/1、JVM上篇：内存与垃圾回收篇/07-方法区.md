...
![image-20210510141044840](images/07-方法区/b802b35c3936900c290f8ad123e68b9b.png)

从线程共享与否的角度来看

![image-20210510141131860](images/07-方法区/f356e103d9f75e3c0086079bba2b28a9-1703780219668-3.png)

## 7.1. 栈、堆、方法区的交互关系

![image-20200708094747667](images/07-方法区/b9f11764ee47b28d37f7764dfd9c9f55.png)
...
![image-20210510195446194](images/07-方法区/ec22e5d6086dd6219195b2d886118d65.png)
...
![image-20200708095853544](images/07-方法区/6a7f2350e0f4e0cde0ac246225e2acdd.png)

### 7.2.2. 方法区的基本理解

- 方法区（Method Area）与 Java 堆一样，是`各个线程共享的内存区域`。
...
![image-20210510142516373](images/07-方法区/fb71df89c52f89d3b711e0b648de244c.png)
...
![image-20210510142656677](images/07-方法区/7d0789c206d53bfb4b6004052236197a.png)

而到了 JDK8，终于完全废弃了永久代的概念，改用与 JRockit、J9 一样在**本地内存中实现的元空间（Metaspace）来代替**

![image-20200708103055914](images/07-方法区/1c229dc39ffc79e8e4f3abf765378d3b.png)
...
![image-20200708111756800](images/07-方法区/79ec6c83a9b6a4a7281c2ef5442f18ce.png)
...
![image-20210510143959924](images/07-方法区/6d1f5d0e7f035df53d16a16451781677.png)
...
![image-20200708161728320](images/07-方法区/8a5fcba10ccb773f185288ac22bde206.png)
...
![image-20200708161856504](images/07-方法区/fbe3915506e7979c7d591d17c216fbb1.png)
...
` javap -p -v .\MethodInnerStrucTest.class > mytest.txt`  反编码MethodInnerStrucTest.class 并重定向到mytest.txt输出
...
![image-20200708171151384](images/07-方法区/7ec9b450764c623c89e7de7e0d3c8d19.png)
...
![image-20200708172357052](images/07-方法区/be6d675673b353a266d48df43b0411f4.png)
...
![image-20210510145947122](images/07-方法区/86cc8ce43ce5bcc9b2b167188b839a1b.png)
...
![image-20210510151436251](images/07-方法区/b5b9e7f8efd1cf8e889650cea5318b61.png)

![image-20210510151504259](images/07-方法区/84396fa6be6164ffc54868e78c59dde6.png)

![image-20210510151520952](images/07-方法区/2214af3dfe294eec73899c935589f569.png)

![image-20210510151609566](images/07-方法区/64b6b95d37e97205b2f23a089addf321.png)

![image-20210510151648231](images/07-方法区/692908400668791ac383561affed3520.png)

![image-20210510151712355](images/07-方法区/885b0a628db8e5d96bd270b6c1073bc6.png)

![image-20210510151753579](images/07-方法区/922d24be85f3ac3bf951e81bc1101e3d.png)

![image-20210510151829404](images/07-方法区/67615b1384f553cab185a32460e607eb.png)

![image-20210510151918342](images/07-方法区/4b80397e4e7a553cc7d7f0f9a8b38d1d.png)

![image-20210510151951327](images/07-方法区/bcc93f31b5dc95f4676dbd49ecc095d7.png)

![image-20200708205708057](images/07-方法区/dfba814e0c1b198df7175fe93284c3e0.png)

![image-20210510152102989](images/07-方法区/35be7c27bee2aa8c0930cffbefa07cd8.png)

![image-20210510152138492](images/07-方法区/5f2935ec0858f5a74f512018e582f19c.png)

![image-20210510195824437](images/07-方法区/f584ed9e2b6d99499b2a451a63f12449.png)

![image-20210510195911639](images/07-方法区/b4428f7f351048601296fe211c40185e.png)

![image-20210510152243933](images/07-方法区/bf50cc494594b4588794984df6448303.png)

## 7.6. 方法区的演进细节

1. 首先明确：**只有 Hotspot 才有永久代**。
   + BEA JRockit、IBMJ9 等来说，是不存在永久代的概念的。原则上如何实现方法区属于虚拟机实现细节，不受《Java 虚拟机规范》管束，并不要求统一
2. Hotspot 中方法区的变化：
...
![image-20200708211541300](images/07-方法区/1a3aa55257c3150d78327542e5ca230e.png)

![image-20200708211609911](images/07-方法区/e0f65fc4228d9b6573ae1b23d9a1558b.png)

![image-20200708211637952](images/07-方法区/c3ed969b0d2bad704c22481208e5dd10.png)

### 7.6.1. 为什么永久代要被元空间替代？

官网地址：[JEP 122: Remove the Permanent Generation (java.net)](http://openjdk.java.net/jeps/122)

![image-20210510163843564](images/07-方法区/36feeb020d5de3a8c31056f27b27efc4.png)
...
使用 JHSDB 工具【jdk9后引入】进行分析，这里细节略掉

![image-20200708215218078](images/07-方法区/e8ced63603ee4a62c9eb7d4c48ae94dd.png)

staticobj 随着 Test 的类型信息存放在方法区，instanceobj 随着 Test 的对象实例存放在 Java 堆，localobject 则是存放在 foo()方法栈帧的局部变量表中。

![image-20200708215025527](images/07-方法区/e7c6b3dd5175466ee891da9e84577418.png)
...
![image-20200708220303243](images/07-方法区/f73315a70302396e391a532dcf87ce26.png)
...
>
> 答：年轻代中分为一个Eden区和两个Surviver区，**比例为8：1：1**
>
> 
>
> <mark>小米</mark>：
...
>
> 答：1、新生代：年轻代主要存放新创建的对象，内存大小相对会比较小，垃圾回收会比较频繁。年轻代分成1个Eden Space和2个Suvivor Space（from 和to）。 2、老年代：年老代主要存放JVM认为生命周期比较长的对象（经过几次的新生代的垃圾回收后仍然存在），内存大小相对会比较大，垃圾回收也相对没有那么频繁
>
> <mark>字节跳动</mark>：
>
> 二面：Java 的内存分区
>
> 二面：讲讲j vm 运行时数据库区 什么时候对象会进入老年代？
>
> 答：
>
> 1. (1) 迭代年龄判断（静态年龄）
...
>    
>
>    **在对象的对象头信息中存储着对象的迭代年龄（MaxTenuringThreshold）**，迭代年龄会在每次**YoungGC**之后对象的移区操作中增加，**每执行一次MiniorGC**。当这个年龄大于到 **15(默认)** 之后，**这个对象将会被移入老年代**.
>
>    
>
>    **可以通过这个参数设置这个年龄值**：
...
>
> 答：Java8相对于之前的版本，引入了许多新特性和改进。其中一些与内存模型相关的改进包括了对堆的优化和元空间 的引入。 2.1 堆的优化 在Java8之前，堆的结构相对简单，由一个或多个连续的堆区域组成。而在Java8中，引入了元空间（Metaspac e），取代了永久代（PermGen）。 元空间是方法区的替代品，用于存储类的相关信息。相比之下，元空间具有以下优势： 动态分配内存：元空间的大小不再固定，它可以根据需要动态地分配和释放内存。 内存回收：元空间的内存由垃圾收集器（Garbage Collector）进行管理，不再需要手动设置永久代的大小。
>
> 
>
> <mark>美团</mark>：

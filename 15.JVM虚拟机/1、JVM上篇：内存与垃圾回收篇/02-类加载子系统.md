...
![image-20200705080719531](images/02-类加载子系统/e4bc9ed374db7f35e68f23f4813205bd-1703783198668-79.png)

详细版本

![image-20221215005255047](images/02-类加载子系统/image-20221215005255047-1703783198668-80.png)
...
![image-20200705081813409](images/02-类加载子系统/3569bfb903e80b66ee7e972a6b4a5036-1703783198668-81.png)
...
![image-20200705081913538](images/02-类加载子系统/e8172076eaa7a152408633a353f06b2c-1703783198668-82.png)
...
![image-20200705082255746](images/02-类加载子系统/8cc54647114c456695ac352336c74600-1703783198668-83.png)

### 1、加载阶段loading

![image-20200705082601441](images/02-类加载子系统/a9497a1eeb7fae3022846b509186fdcd-1703783198668-84.png)
...
- 此方法不需定义，是 <u>javac 编译器自动收集类中的所有类变量的赋值动作和静态代码块中的语句合并而来</u>。
- 构造器方法中指令按显示赋值语句和静态代码块在源文件中出现的顺序执行。
...
![image-20200705094149223](images/02-类加载子系统/1e553c6d5254f827d2dfab537bea3ab9-1697475388979-3-1703783198668-85.png)
...
- 这个类加载使用 **C/C++语言**实现的，嵌套在 JVM 内部。
- 它用来**加载 Java 的核心库**（JAVA_HOME/jre/lib/rt.jar、resources.jar 或 sun.boot.class.path 路径下的内容），用于提供 JVM 自身需要的类
...
![image-20200705103516138](images/02-类加载子系统/876534b3c2f447d3fc33e6f1db218068-1703783198668-86.png)

`sun.misc.Launcher` 它是一个 java 虚拟机的入口应用

![image-20200705103636003](images/02-类加载子系统/a22114b608dffe484041b591d486a7fd-1703783198668-87.png)
...
![image-20200705105151258](images/02-类加载子系统/05fa27fcc38eeaaa5babff55a00882a3-1703783198668-88.png)
...
![image-20200705105810107](images/02-类加载子系统/bed320014f52bb27c8f3d795b3dc3b4a-1703783198668-89.png)
...
- 保护程序安全，防止核心 API 被随意篡改
  - 自定义类：java.lang.String
    ...
    package java.lang;
    //自己模块下String类
    //创建此对象时：报错找不到main方法，因为String属于java.lang包，被Bootstrap CLassLoader加载，核心包中String无main方法
    public class String {
        //
        static{
...
自定义 String 类，但是在加载自定义 String 类的时候会率先使用引`导类加载器`加载，而引导类加载器在加载的过程中会先加载 jdk 自带的文件（rt.jar 包中 java\lang\String.class），报错信息说没有 main 方法，就是因为加载的是 rt.jar 包中的 string 类。这样可以保证对 java 核心源代码的保护，这就是沙箱安全机制。

...
以 String 为例：2 个 int 值共占 8 字节，对象引用占用 4 字节，对象头 8 字节，类型指针4字节 合计 24字节，故占 24 字节。（jdk7 中）
...
![img](https://img2022.cnblogs.com/blog/1708060/202204/1708060-20220426145509634-60933669.png)

如上图所示，A 对象浅堆大小 1 KB，B 对象 2 KB，C 对象 100 KB。A 对象同时引用了 B 对象和 C 对象，但由于 C 对象也被 D 引用，所以 A 对象的深堆大小为 3 KB（1 KB + 2 KB）。

A 对象实际大小（1 KB + 2 KB + 100 KB）> A 对象深堆 > A 对象浅堆。
...
public class MemoryLeak {
    ...
    public void oomTests(){
        Object obj＝new Object();//局部变量 本来方法栈结束后就释放，但是被static容器持有
        list.add(obj);
    }
}
```
...
改变哈希值，当一个对象被存储进 HashSet 集合中以后，就**不能修改这个对象中的那些参与计算哈希值的字段**了。
...
public class ChangeHashCode1 {
    public static void main(String[] args) {
        ...
        System.out.println("hs.remove = " + hs.remove(cc));//false 41存进去，按51取失败 导致内存泄漏
        hs.add(cc);
...
public class ChangeHashCode {
    public static void main(String[] args) {
        HashSet set = new HashSet();// hash定位槽位，槽位相同equals比较  原因：减少全部判重
        ...
        set.add(new Person(1001, "AA"));//与p1的哈希后的槽位一样，但是equals比较时不一样
        System.out.println(set);// 大小为4
...
对于这个问题，可以使用 [WeakHashMap  弱引用]代表缓存，此种 Map 的特点是，当除了自身有对 key 的引用外，此 key 没有其他引用那么此 map 会自动丢弃此值。
...
![image-20240403005238356](images/补充：浅堆深堆与内存泄露/image-20240403005238356.png)
...
public class Stack {
    ...
    public Object pop() { //出栈
        ...
        return elements[--size];//只操作了指针，当前位置引用依旧存在，优化：需要手动置null
    }
...
![image-20240403005926642](images/补充：浅堆深堆与内存泄露/image-20240403005926642.png)
...
![image-20240403010343943](images/补充：浅堆深堆与内存泄露/image-20240403010343943.png)

![image-20240403010432333](images/补充：浅堆深堆与内存泄露/image-20240403010432333.png)

![image-20240403010454830](images/补充：浅堆深堆与内存泄露/image-20240403010454830.png)

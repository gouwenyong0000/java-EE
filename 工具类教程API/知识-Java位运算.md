...
- 符号位（最高位）权重为\(-2^7=-128\)；
- 有效数值位对应权重：\(2^6=64\)、\(2^4=16\)、\(2^2=4\)、\(2^1=2\)；
- 计算结果：\(-128 + 64 + 16 + 4 + 2 = -42\)。

#### 3. 补码表示小数（定点数）

通过调整 “隐含小数点” 的位置，补码可表示小数：以`1101.0110`为例：

- 符号位对应整数部分最高位，权重为\(-2^3=-8\)；
- 整数部分有效位权重：\(2^2=4\)、\(2^0=1\)；
- 小数部分有效位权重：\(2^{-2}=0.25\)、\(2^{-3}=0.125\)；
- 计算结果：\(-8 + 4 + 1 + 0.25 + 0.125 = -2.625\)。

#### 补码的作用

补码是计算机中表示有符号数的核心方式，其优势在于**将减法运算转化为加法运算**（通过 “取反加 1” 得到补码），大幅简化了硬件的运算电路设计，是数字系统中数值表示的基础方案。
...
> 这是 Java 处理负数时的关键点。
>
> - **`>>`（算术右移）**：保留符号位。如果左边最高位是 1（负数），移位后左边补 1；如果是 0（正数），补 0。
>   - *数学意义*：相当于除以 $2^n$ 并向下取整。
> - **`>>>`（逻辑右移/无符号右移）**：不考虑符号位。无论正负，左边一律补 0。
>   - *应用场景*：在处理颜色值 (ARGB)、Hash 算法、或将底层字节流视为无符号数处理时，必须使用 `>>>`。

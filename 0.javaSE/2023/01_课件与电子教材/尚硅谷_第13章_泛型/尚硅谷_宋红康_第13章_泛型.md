...
⑤ **除创建泛型类对象外，子类继承泛型类时、实现类实现泛型接口时，也可以确定父类/接口中泛型结构中的泛型参数。**
...
⑤ **在类/接口上声明的泛型，在本类或本接口中即代表某种类型，但不可以在静态方法中使用类的泛型。**[实例化时才能确定泛型的具体类型]
...
public class DAO<T> {
    ...
    //返回 map 中存放的所有 T 对象
    public List<T> list(){
        //错误的： values.getClass();  class java.util.HashMap$Values和List不具备父子关系
...
2. 类SuperA是类A的父类，则G<SuperA> 与 G<A>的关系：G<SuperA> 和 G<A>是并列的两个类，没有任何子父类的关系。
...
public void test1(){
    ...
    //可以读取数据类型为父类，但是不能写入数据   因为添加范围是（-∞，F]，即子类可以无限小，但是父类不能赋给更小子类，
...
public void test2(){
    ...
    //可以读取数据类型为Object，能写入数据   因为添加范围是[F，+∞)，即子类可以下限一定

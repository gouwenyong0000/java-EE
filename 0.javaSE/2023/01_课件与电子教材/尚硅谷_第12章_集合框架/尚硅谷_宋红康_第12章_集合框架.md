...
（5）`boolean contains(Object obj)`：判断当前集合中是否存在一个**与obj对象equals**返回true的元素
...
（8）void clear()：清空集合元素
（9） `boolean remove(Object obj)` ：从当前集合中**删除第一个找到的与obj对象equals返回true**的元素。
（10）`boolean removeAll(Collection coll)`：从当前集合中删除所有与coll集合中相同的元素。即`this = this - this ∩ coll`
（11）`boolean retainAll(Collection coll)`：从当前集合中删除两个集合中不同的元素，使得当前集合仅保留与coll集合中的元素相同的元素，即当前集合中**仅保留两个集合的交集**，即`this  = this ∩ coll`；

> 拓展：
>
> A、在JDK8.0时，Collection接口有了removeIf 方法，即可以根据条件删除   底层调用iterator的remove
>
> B、Stream 的filter进行过滤
...
public class TestCollectionRemove {
    ...
    public void test03() {
        ...
        System.out.println("coll.retainAll(other)之后，other = " + other);//[小李广, 扫地僧, 尚硅谷]
    }
...
// 注意 基本数据类型  Arrays.asList入参只能是Object类型
int[] ns = new int[]{1, 2, 3};
List<int[]> list1 = Arrays.asList(ns);
System.out.println("list1 = " + list1);//list1 = [[I@4dd8dc3]
System.out.println("list1.size() = " + list1.size());// list1.size() = 1
```
...
- Iterator接口的常用方法如下：
  ...
  - `void remove()`   从列表中删除next或previous返回的最后一个元素（可选操作）。每次调用next或previous只能进行一次此调用。只有在最后一次调用next或previous之后还没有调用add情况下才能进行。
...
- 对Vector、ArrayList在迭代的时候如果同时对其进行修改就会抛出ConcurrentModificationException异常

```java
//迭代器remove 源码  检查获取迭代器后集合有没被修改过   确保数据一致性
public void remove() {
            if (lastRet < 0)
    ...
    modCount：当前集合增删改次数  
    expectedModCount：获取迭代器时的modCount快照次数，
    */
...
![image-20230526014132721](.\images\image-20230526014132721.png)
...
- 删除和替换元素
  - `Object remove(int index)`:移除指定index位置的元素，并返回此元素,后面元素前移
...
//拓展：如果想要移除元素2    list.remove(Integer.valueOf(2)); 
```
...
java.util.Collection:存储一个一个的数据
    |-----子接口：Set:存储无序的、不可重复的数据(高中学习的集合)
           |---- HashSet：主要实现类；底层使用的是HashMap，即使用数组+单向链表+红黑树结构进行存储。（jdk8中）
                |---- LinkedHashSet：是HashSet的子类；在现有的数组+单向链表+红黑树结构的基础上，又添加了 一组双向链表，用于记录添加元素的先后顺序。即：我们可以按照添加元素的顺序实现遍历。便于频繁的查询操作。
...
- 对于存放在Set容器中的对象，**对应的类一定要重写hashCode()和equals(Object obj)方法**，以实现对象相等规则。即：**“相等的对象必须具有相等的散列码”**。
  - **分析：不可重复需要全量判断对象是否相等，数据规模过大时遍历性能差。先通过hashcode值快速取模分组定位，再通过equals精确比较。 **

  - 为什么重写equals前需要重写hashcode？
...
#### 5.2.6 面试题 -- 理解hashCode 和和equal

> 知识点：对于存放在Set容器中的对象，**对应的类一定要重写hashCode()和equals(Object obj)方法**，以实现对象相等规则。即：**“相等的对象必须具有相等的散列码”**。
>
> - **分析：不可重复需要全量判断对象是否相等，数据规模过大时遍历性能差。先通过hashcode值快速取模分组定位，再通过equals精确比较。 **
...
set.add(p2);
p1.name = "CC";
set.remove(p1);//不能删除 判断HashSet相等： hashcode定位 +equal精确判断   
//删除时计算hashcode值 时1001 + CC 计算，添加时计算hashcode是根据1001 + AA 计算，两者hash值不相等
System.out.println(set);

set.add(new Person(1001,"CC"));
System.out.println(set);//可以添加 p1 添加是1001 + AA 定位  ，new Person(1001,"CC") 定位是1001 + "CC",取模值不同

set.add(new Person(1001,"AA"));
System.out.println(set);// 可以添加  和P1 hash值相同 但是equal不同
...
public LinkedHashSet() {
	...
	// hashSet中构造器  使用LinkedHashMap保存
    HashSet(int initialCapacity, float loadFactor, boolean dummy) {
        map = new LinkedHashMap<>(initialCapacity, loadFactor);
    } 
    
//使用LinkedHashMap 中Entry  包含前后节点
...
- Map 中的 `key彼此之间是不可重复的、无序的,用Set来存放`，`不允许重复`，即同一个 Map 对象所对应的类，须重写hashCode()和equals()方法

  - **思想：唯一性--通过hash值快速分组定位槽位，相同槽位通过equals进行精确比较  ，减少比较规模。**
  - 前提：相同对象的hash值一致
  - **注意点：**重写hashCode和eques方法后，hashCode、eques结果会和属性值相关；修改属性会导致hashCode定位的槽位变化，不能正确处理，造成内存泄漏。

- key 和 value 之间存在单向一对一关系，即通过指定的 key 总能找到唯一的、确定的 value，不同key对应的`value可以重复`，所有的value就构成一个Collection集合。value所在的类要重写equals()方法。

  <img src="images/image-20220514190412763.png" alt="image-20220514190412763" style="zoom:67%;" />

- key和value构成一个entry。所有的entry彼此之间是`无序的`、`不可重复的`。所有的entry就构成了一个Set集合。
...
- **添加、修改操作：**
  - Object put(Object key,Object value)：将指定key-value添加到(或修改)当前map对象中，**返回值为旧值**
...
public class TestMapMethod {
    public static void main(String[] args) {
        ...
        map.put("黄晓明", "杨颖");
        map.put("李晨", "李小璐");
        Object oldValue = map.put("李晨", "范冰冰");
        System.out.println("oldValue = " + oldValue);//oldValue = 李小璐
        map.put("邓超", "孙俪");
        System.out.println(map);//{邓超=孙俪, 李晨=范冰冰, 黄晓明=杨颖}

        //删除指定的key-value
        System.out.println(map.remove("黄晓明"));
        System.out.println(map);//杨颖
...
- HashMap `判断两个key相等的标准`是：两个 key 的hashCode值相等，通过 equals() 方法返回 true。
  - **思想：唯一性--通过hashCode值快速分组定位槽位，相同槽位通过equals进行精确比较  ，减少比较规模。**
  - 前提：相同对象的hash值一致
  - **注意点：**重写hashCode和eques方法后，hashCode、eques结果会和属性值相关；修改属性会导致hashCode定位的槽位变化，不能正确处理，造成内存泄漏。
...
考点：判断两个key相等  参考6.3.1

```java
public class Call {
    public static void main(String[] args) {

        AAA aa = new AAA("AA", 1001);
        AAA bb = new AAA("AA", 1002);
        AAA cc = new AAA("AA", 1002);

        Map<Object, Object> map = new HashMap<>();
        map.put(aa,1);
        map.put(bb,1);
        map.put(cc,1);
        System.out.println("map = " + map);//map = {AAA{name='AA', age=1002}=1, AAA{name='AA', age=1001}=1}


        System.out.println("aa.hashCode() = " + aa.hashCode());//aa.hashCode() = 66442
        aa.name = "bb";
        System.out.println("aa.hashCode() = " + aa.hashCode());//aa.hashCode() = 99178
        System.out.println("map.containsKey(aa) = " + map.containsKey(aa));//map.containsKey(aa) = false
...
- int binarySearch(List list,T key)在List集合中二分查找某个元素的下标，但是List的元素必须是T或T的子类对象，而且必须是可比较大小的，即支持自然排序的。而且集合也事先必须是有序的，否则结果不确定。
- int binarySearch(List list,T key,Comparator c)在List集合中查找某个元素的下标，但是List的元素必须是T或T的子类对象，而且集合也事先必须是按照c比较器规则进行排序过的，否则结果不确定。
- int frequency(Collection c，Object o)：返回指定集合中指定元素的出现次数  --频率

...
public class TestAnnotation {
    public static void main(String[] args) {
        ...
        for (Field declaredField : declaredFields) {
            ...
            if(column!= null) {
                columns[index++] = column.columnName();//获取注解属性值  Column注解中 columnName 的值
            }
        }
...
* <u>`public static Integer valueOf(String s,int radix)` ：根据传递的字符串和进制创建一个Integer对象    示例： `Integer.valueOf("1f", 16)//31`</u>
...
boolean b = Boolean.valueOf("true或false");//非 忽略大小写true都为false
...
#### 11.6.5 byte[]、int、float转换

```java
    /**
     * int整数转换为4字节的byte数组
     * @param i 整数  @return byte数组
     */
...
Integer n = 1;//这个用的是缓冲的常量对象，在方法区    自动装箱valueOf()
...
Integer i = 1;
Double d = 1.0
System.out.println(i==d);//编译报错  对象比较时类型必须相同或者具有父子类关系
```

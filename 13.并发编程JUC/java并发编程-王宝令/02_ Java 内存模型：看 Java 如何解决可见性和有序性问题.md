...
[![](image/02_ Java 内存模型：看 Java 如何解决可见性和有序性问题/b1fa541e98c74bc2a033d9ac5ae7fbe1.png)](https://static001.geekbang.org/resource/image/b1/e1/b1fa541e98c74bc2a033d9ac5ae7fbe1.png)
...
1. 声明共享变量abc，并使用`volatile关键字`修饰abc
2. 声明共享变量abc，在`synchronized关键字`对abc的赋值代码块加锁，由于Happen-before管程锁的规则，可以使得后续的线程可以看到abc的值。
3. A线程启动后，使用A.JOIN()方法来完成运行，后续线程再启动，则一定可以看到abc==3，`线程间阻塞等待`。

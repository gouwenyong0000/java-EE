...
>
> `sizeCtl`为正数，如果数组未初始化，那么其记录的是数组的初始容量，如果数组已经初始化，那么其记录的是数组的扩容阈值（数组的初始容量 * 0.75）
>
...
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    //基于key计算hash值，并进行一定的扰动 这个值一定是一个正数，方便后面添加元素判断该节点的类型
    ...
    for (Node<K,V>[] tab = table;;) {
        ...
        //如果hash计算得到的桶位置元素的hash值为MOVED，证明正在扩容，那么协助扩容
            ...
            V oldVal = null;
            //对当前桶（获取头结点的锁）进行加锁，保证线程安全，执行元素添加操作
            synchronized (f) {
                if (tabAt(tab, i) == f) {// 再次判断，避免倍其他线程树化
                    //普通链表节点
                    if (fh >= 0) {
                        binCount = 1;
                        // 循环遍历链表，如果值存在则覆盖，不存在就链到尾部
...
![](./images/ConcurrentHashMap源码分析(二)/Snipaste_2020-05-16_12-04-08-1756829867232-2.jpg)
...
![](./images/ConcurrentHashMap源码分析(二)/微信截图_20200516213316.png)
...
![](./images/ConcurrentHashMap源码分析(二)/微信截图_20200516215323.png)
...
![image-20250904012704375](./images/ConcurrentHashMap源码分析(二)/image-20250904012704375.png)

> 这个图是 **`ConcurrentHashMap` 的长度统计（元素个数统计）机制** 的示意图，和 `LongAdder` 的实现原理类似。
>
> ------
>
> 图解说明：
>
> - **ConcurrentHashMap**
>   - 内部维护了一个 **`baseCount`** 变量（基础计数器）。
>   - 还维护了一组 **`CounterCell`**（分段计数器数组）。
> - **baseCount**
>   - 是一个全局的基础计数值。
>   - 当有线程要增加或减少 map 的元素数量时，会 **先尝试对 `baseCount` 做一次 CAS（Compare-And-Swap）更新**。
>   - 如果 CAS 成功，就说明当前竞争不大，直接更新完成。
> - **CounterCell[]**
>   - 如果 **`baseCount` 的 CAS 更新失败（说明竞争比较激烈，多线程同时修改）**，那么线程就会根据自己的哈希或线程 ID **定位到某一个 `CounterCell`**，然后对这个 cell 进行 CAS 自增。
>   - 这样就把并发冲突分散到多个 `CounterCell` 上，降低了竞争热点。
> - **最终统计总数**
>   - 要获取整个 map 的大小时，`ConcurrentHashMap.size()` 会把：`baseCount + 所有 CounterCell 的 value `之和作为最终的集合长度。
>
> ------
>
> ✅ 这个机制的好处：
>
> - 在低并发时，直接更新 `baseCount` 就够了，开销小。
> - 在高并发时，通过分散到多个 `CounterCell`，避免了所有线程都去抢同一个 `baseCount` 锁，提高了吞吐量。
...
![](./images/ConcurrentHashMap源码分析(二)/微信截图_20200516185407.png)
...
public class Test01 {
    public static void main(String[] args) throws Exception {
        final ConcurrentHashMap chm = new ConcurrentHashMap(32);// 出事容量64

        //  64*0.75 = 48 先添加47个触发扩容
        for (int i = 0; i < 47; i++) {
            chm.put("abc"+i,i);
        }
...
//        String s = "通话";
//        String s2 = "重地";
//        System.out.println(s.hashCode()); //哈市值相同

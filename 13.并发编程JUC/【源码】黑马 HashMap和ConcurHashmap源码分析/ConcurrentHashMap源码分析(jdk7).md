...
`putOrderedObject`：设置对象的属性值或者数组中某个角标的元素，不保证线程即时可见性，但是更高效
...
![](./images/ConcurrentHashMap源码分析(一)/微信截图_20200417093442-1756829817873-2-1756829825003-4.png)
...
public ConcurrentHashMap(int initialCapacity,
    ...
    Segment<K,V> s0 =
        new Segment<K,V>(loadFactor, (int)(cap * loadFactor),
                         (HashEntry<K,V>[])new HashEntry[cap]);//每个Segment内Entry数组
    Segment<K,V>[] ss = (Segment<K,V>[])new Segment[ssize];
    //利用Unsafe类，将创建的Segment对象存入0角标位置，其余要使用时在复制[0]模板
...
> 初始化时只有Seqment[0] 初始化，其余要使用时在复制[0]模板

![](./images/ConcurrentHashMap源码分析(一)/微信截图_20200417101208.png)
...
> `int j = (hash >>> segmentShift) & segmentMask;`
>
> 补充说明：如果用hash值的全部位作为hash槽计算，内部hashEntry大概率也是在同一个位置，冲突概率高
...
![](./images/ConcurrentHashMap源码分析(一)/微信图片_20200417111509.png)

![](./images/ConcurrentHashMap源码分析(一)/微信截图_20200417111424.png)
...
![](./images/ConcurrentHashMap源码分析(一)/微信截图_20200417111834.png)
...
![](./images/ConcurrentHashMap源码分析(一)/微信截图_20200417113823.png)

> 图二

![](./images/ConcurrentHashMap源码分析(一)/微信截图_20200417114240.png)

> 图三

![](./images/ConcurrentHashMap源码分析(一)/微信截图_20200417114954.png)

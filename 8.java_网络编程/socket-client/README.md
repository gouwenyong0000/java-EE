...
- 支持指数退避重连：`reconnectInterval * 2^(attempt-1)`，最大 30 秒
...
> **注意**：两套模式是**独立的**，一帧数据会同时进入回调和队列，互不干扰。

### 5. 字符 + 字节双重支持

- `Response.text()` —— 文本协议直接读（按协议 charset 解码）
- `Response.bytes()` —— 完整帧原始字节（含协议层附加字段）
- `Response.body()` —— 业务载荷（已剥离协议层字段）
...
- `ArrayBlockingQueue(256)` 有界队列：消费者慢于生产者时，队列满则丢弃最旧数据
- `pendingFrames(1024)` 上限：超过时丢弃最旧帧，防止 OOM
- 两层背压保护确保客户端在极端情况下不会崩溃

### 8. 指数退避重连

```java
// 重连间隔计算公式
long backoffMs = Math.min(baseMs * (1L << attempt), 30_000L);
// attempt=0: 1s, attempt=1: 2s, attempt=2: 4s, ... 最大 30s
```
...
```bash
# 方式一：直接运行编译后的类
java -cp target/classes com.example.cli.TestServer

# 方式二：Maven 运行
mvn -q exec:java -Dexec.mainClass="com.example.cli.TestServer"
```

启动后会监听：
- `127.0.0.1:5025` —— LineProtocol（CRLF 文本 SCPI）
- `127.0.0.1:9000` —— LengthFieldProtocol（二进制帧 AA + LEN + PAYLOAD + CRC8）
...
```bash
java -cp target/classes com.example.cli.Demo
```

Demo 会同时演示 SocketConnectionManager 多连接管理 + SocketClientITFImpl 单连接直用两种模式。

### 4. 无 Java 环境？用 Python 模拟服务端

```bash
python mock_instrument.py
```

Python 版模拟服务端（仅支持 5025 文本端口），行为与 TestServer 一致。
...
<!-- pom.xml 核心依赖 -->
<dependencies>
  <!-- SLF4J 日志门面（运行时需要搭配具体实现如 logback-classic） -->
  <dependency>
...
> **注意**：项目 `pom.xml` 已内置 `logback-classic` 作为日志实现，
> 配置文件位于 `src/main/resources/logback.xml`。
> 如需自定义日志级别或输出格式，修改该文件即可。
...
如果现有 LineProtocol / LengthFieldProtocol 不满足需求，可实现自定义协议：

```java
public class MyProtocol implements Protocol {
    // 实现 encode(Command) → byte[]：把命令编码成服务端能懂的帧格式
    // 实现 decode(byte[], int, int) → List<Response>：从原始字节流切出完整响应帧
    // 实现 charset() → Charset：返回字符编码
    // 实现 clearCache()：清空内部跨包缓存
}
```

关键约束：
1. **对称编解码**：`decode(encode(cmd))` 必须能还原原始命令对应的响应
2. **跨包缓存**：TCP 流式协议，必须处理半帧/粘包
3. **线程安全**：使用 `synchronized` 或其他同步机制保护共享状态

### 扩展示例：自定义 SCPI 命令封装

```java
public class InstrumentDriver {
    private final ClientITF client;

    public InstrumentDriver(ClientITF client) {
        this.client = client;
    }

    // 封装电压测量命令
    public double measureVoltage() {
        Response resp = client.sendAndRegex("MEAS:VOLT?", "VOLT:([0-9.]+)", null);
        return Double.parseDouble(
            Pattern.compile("VOLT:([0-9.]+)").matcher(resp.text()).group(1)
        );
    }

    // 封装输出开关
    public void setOutput(boolean on) {
        client.sendAndRegex("OUTP " + (on ? "ON" : "OFF"), "OK", null);
    }
}
```
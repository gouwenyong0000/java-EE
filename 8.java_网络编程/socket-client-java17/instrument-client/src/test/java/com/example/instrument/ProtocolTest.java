...
class ProtocolTest {
  ...
  class LengthFieldProtocolTests {
    ...
    void testDecodeSingleBinaryFrame() {
      ...
      assertEquals(1, frames.size());
      assertArrayEquals(payload, frames.get(0).bytes());
    }
    ...
    void testDecodeBinarySticky() {
      ...
      assertEquals(2, frames.size());
      assertArrayEquals(new byte[]{0x01}, frames.get(0).bytes());
      assertArrayEquals(new byte[]{0x02, 0x03}, frames.get(1).bytes());
    }

    @Test
    @DisplayName("decode：拆包分两次到达应重组")
    void testDecodeBinaryFragmented() {
      byte[] frame = p.encode(new Command(new byte[]{(byte) 0xAA, (byte) 0xBB, (byte) 0xCC, (byte) 0xDD}));
      ...
      assertEquals(1, second.size());
      assertArrayEquals(new byte[]{(byte) 0xAA, (byte) 0xBB, (byte) 0xCC, (byte) 0xDD}, second.get(0).bytes());
    }

    @Test
    @DisplayName("decode：CRC 校验失败的帧应被丢弃并触发 resync")
    void testDecodeCrcFailureDiscarded() {
      ...
      List<Response> frames = p.decode(frame, 0, frame.length);
      assertTrue(frames.isEmpty(), "CRC 失败的帧应被丢弃");
    }
  ...
  class ImmutabilityTests {
    ...
    void testResponseBytesIsolation() {
      byte[] src = {1, 2, 3};
      Response r = new Response(src, StandardCharsets.UTF_8);
      byte[] got = r.bytes();
      got[0] = 99;

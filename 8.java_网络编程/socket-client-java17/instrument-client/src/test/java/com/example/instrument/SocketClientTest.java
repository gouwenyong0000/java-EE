...
import com.example.instrument.engine.SocketClientException;
...
class SocketClientTest {
  ...
  private static SocketClientConfig customConfig(
    ...
    return new SocketClientConfig(
        ...
        Duration.ofMillis(500), reconnectInterval,
        maxReconnect, true);
  }
  ...
  class CommandTests {
    ...
    void testMeasureVoltage() {
      ...
      assertAll(
          () -> assertNotNull(resp),
          () -> assertTrue(resp.text().contains("VOLT:1.234"))
      );
    }
    ...
    void testStatus() {
      ...
      assertTrue(resp.text().contains("READY"));
    }
  ...
  class BinaryProtocolTests {
    ...
    void testBinaryEcho() {
      ...
      try {
        ...
        byte[] payload = "ECHO:hello".getBytes();
        Response resp = client.sendAndMatch(
            new Command(payload),
            r -> new String(r.bytes()).startsWith("ECHO:hello"),
            MED_TIMEOUT);
        assertTrue(new String(resp.bytes()).startsWith("ECHO:hello"));
      } finally {
        client.disconnect();
      }
    }
    ...
    void testBinaryStickyResponse() {
      ...
      try {
        ...
        Response resp = client.sendAndMatch(
            Command.of("STICKY"),
            r -> new String(r.bytes()).startsWith("STICKY:F1"),
            MED_TIMEOUT);
        assertTrue(new String(resp.bytes()).startsWith("STICKY:F1"));
      } finally {
        client.disconnect();
      }
    }
